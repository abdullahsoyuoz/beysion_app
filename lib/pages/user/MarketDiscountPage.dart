...
class MarketDiscountPage extends StatefulWidget {
  @override
  _MarketDiscountPageState createState() => _MarketDiscountPageState();
}

class _MarketDiscountPageState extends State<MarketDiscountPage> with SingleTickerProviderStateMixin  {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leadingWidth: 0,
        elevation: 1,
        toolbarHeight: AppBar().preferredSize.height* 1.75,
        actions: [
          IconButton(
      ...
      body: campaignMarketDataList.length>0 ? Container(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            CampaignMarketEntity campaignMarketData = campaignMarketDataList[index];
            return CampaignWidgetView0(campaignMarketData, index);
          }),
                              ...
                              children: [
                                Container(
                                  ...
                                  getData == false ? Container(
                                      ...
                                      itemCount: campaignMarketSearchDataList.length,
                                      itemBuilder: (context, index){
                                        CampaignMarketEntity marketDataEntity = campaignMarketSearchDataList[index];
                                        return InkWell(
                                          onTap: (){
                                            Navigator.pop(context);
                                            marketCampaignSuchenDialogController = new TextEditingController();
                                            testController = new TextEditingController();
                                            ...
                                            settingRepo.pageViewState.value.pageController.animateToPage(5, duration: Duration(milliseconds: 100), curve: Curves.easeInOutExpo);
                                            //Navigator.push(context, MaterialPageRoute(builder: (context) => MarketProductDiscountPage(marketDataEntity),));
                                          },
                                          child: Container(
          ...
          children: [
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text("${widget.campaignMarketDataEntity.name}", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500),),
                      Container(
                        width: getSize(context, true, 35/360),
                        height: getSize(context, true, 20/360),
                        margin: EdgeInsets.only(left: 10),
                        ...
                        child: Center(child: Text("${widget.campaignMarketDataEntity.avgScore}", style: TextStyle(color: BeysionColors.purple, fontWeight: FontWeight.w700),)),
                      )
                    ],
                  ),
                  Padding(
                    padding: EdgeInsets.only(top: 4.0),
                    child: Text("${widget.campaignMarketDataEntity.fulladdress}", textAlign: TextAlign.center, maxLines: 2, style: TextStyle(color: Colors.grey.shade700, fontSize: 11),),
                  )
                ],
              ),
            ),
                ...
                images: widget.campaignMarketDataEntity.products.map((productEntity) =>InkWell(
                  onTap: (){
                    ...
                    oldProductEntityData.selected = false;
                        Navigator.push(context, MaterialPageRoute(builder: (context) => ProductDetailPage(oldProductEntityData, marketId: widget.campaignMarketDataEntity.id,),));
                  },
                  child: Column(
                    children: [
                      Container(
                      ...
                      Padding(
                        ...
                        child: productEntity.discountPrice == null
                          ...
                          children: [
                            Container(
                              width: getSize(context, true, 40/360),
                              height: getSize(context, true, 30/360),
                              decoration: BoxDecoration(
                                  color: BeysionColors.red,
                                  borderRadius: BorderRadius.circular(5)
                              ),
                              child: Center(child: Text("${fiyatYuzdeHesaplamaCampaign(productEntity)}", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold),)),
                            ),
                            Padding(
                              padding: EdgeInsets.only(left: 5.0),
                              child: Column(
                                children: [
                                  Text("${productEntity.price.toString().replaceAll(".", ",")} €", style: TextStyle(decoration: TextDecoration.lineThrough, color: Colors.grey.shade500),),
                                  Text("${productEntity.discountPrice.toString().replaceAll(".", ",")} €", style: TextStyle(color: Colors.black, fontSize: 18),),
                                ],
                                crossAxisAlignment: CrossAxisAlignment.start,
                              ),
                            )
            ...
            InkWell(
              child: Container(
                ...
                width: getSize(context, true, 230 / 415),
                decoration: BoxDecoration(
                    color: BeysionColors.orange,
                    borderRadius:
                    BorderRadius.all(Radius.circular(5))),
              ...
              onTap: () {
                ...
                settingRepo.pageViewState.value.oneParam = null;
                settingRepo.pageViewState.value.twoParam = null;
                settingRepo.pageViewState.value.oneParam = widget.campaignMarketDataEntity;
                settingRepo.pageViewState.value.pageController.animateToPage(5, duration: Duration(milliseconds: 100), curve: Curves.easeInOutExpo);

                //Navigator.push(context, MaterialPageRoute(builder: (context) => MarketProductDiscountPage(widget.campaignMarketDataEntity),));
              },
            ),
          ],
        ),

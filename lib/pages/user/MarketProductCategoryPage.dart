...
class MarketProductCategoryPage extends StatefulWidget {
  MarketCategoryEntity categoryEntity;
  MarketsAllEntity marketsAllEntity;
  MarketProductCategoryPage(this.categoryEntity, this.marketsAllEntity);
  @override
  _MarketProductCategoryPageState createState() => _MarketProductCategoryPageState();
}

class _MarketProductCategoryPageState extends State<MarketProductCategoryPage> with SingleTickerProviderStateMixin  {
  ...
  List<MarketListEntity> marketListDataNew = new List();
  MarketCategoryEntity selectedCategoryEntity = new MarketCategoryEntity();
  MarketListEntity selectedMarket;
  ...
  void initState() {
    super.initState();
    try{
      var marketListProvider = Provider.of<MarketListProvider>(context, listen: false);
      setState(() {
       marketListDataNew = marketListProvider.marketDataList;
       selectedMarket = marketListDataNew[0];
     });
    }catch(e){

    }
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      Provider.of<MarketInProductsProvider>(context, listen: false).getMarketProductListController(widget.marketsAllEntity.id,pageNumber: page, categoryId: widget.categoryEntity.id);
    });
  ...
  Widget build(BuildContext context) {
    List<ProductEntity> marketProductListData = Provider.of<MarketInProductsProvider>(context, listen: true).marketProductList;
    ...
    return Scaffold(
          ...
          children: [
            InkWell(
              onTap: () {
                ...
                settingRepo.pageViewState.value.oneParam = widget.marketsAllEntity;
            ...
            Padding(
              ...
              child: RichText(text: TextSpan(
                text: "${widget.categoryEntity.name}",
              ...
              Container(
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text("${widget.marketsAllEntity.name}", style: TextStyle(fontSize: 14, fontWeight: FontWeight.normal),),
                    ],
                  ),
                ),
              ),
      ...
      body: marketProductListData.length>0 ? Container(
        ...
        child: ListView(
          ...
          children: [
            if (viewStyle == 0) GridView.builder(
              ...
              itemCount: marketProductListData.length,
              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 2,
                childAspectRatio: 0.57
              ),
              itemBuilder: (context, index) {
                ProductEntity productDataEntity = marketProductListData[index];
                return ProductWidgetView0(productDataEntity, index, widget.marketsAllEntity);
              },
            ) else SizedBox(),
            if (viewStyle == 1) GridView.builder(
              ...
              itemBuilder: (context, index) {
                ProductEntity productDataEntity = marketProductListData[index];
                return ProductWidgetView1(productDataEntity, index, widget.marketsAllEntity);
              },
            ) else SizedBox(),
            if (viewStyle == 2) ListView.separated(
              ...
              itemBuilder: (context, index) {
                ProductEntity productDataEntity = marketProductListData[index];
                return ProductWidgetView2(productDataEntity, index, widget.marketsAllEntity);
              },
            ) else SizedBox(),
          ],
        ),
                                            ...
                                            child: Container(
                                              ...
                                              child: Column(
                                                children: productSearchList.map((productEntity) =>
                                                    InkWell(
                                                      onTap: (){Navigator.push(context, MaterialPageRoute(builder: (context) => ProductDetailPage(productEntity, marketId: widget.marketsAllEntity.id,)));},
                                                      child: Padding(
                                                        padding: const EdgeInsets.all(8.0),
                                                        child: Row(
                                                          crossAxisAlignment: CrossAxisAlignment.center,
                                                                ...
                                                                children: [
                                                                  ...
                                                                  Row(
                                                                    ...
                                                                    children: [
                                                                      ...
                                                                      Expanded(
                                                                        child: Text("${widget.marketsAllEntity.name}",
                                        ...
                                        child: TextField(
                                          ...
                                          onChanged: (value) async {
                                            if(value.length>2){
                                              ...
                                              productSearchList = await getProductAllListSearch(search: value, categoryId: widget.categoryEntity.id);
                                              setStateC(() {
                                              });
                                            }else{
                                              setStateC(() {
  ...
  Future<List<ProductEntity>> getProductAllListSearch({int categoryId = 0, int pageNumber=0, int orderId = 0, String search =""}) async {
    ...
    BaseResponse response = await MarketProductService.operations().marketProducts(widget.marketsAllEntity.id,widget.categoryEntity.id,search,sessionToken,page: pageNumber,order: orderId);
    try{
      getData = false;
  ...
  void cupertinoSelectedMarket() async {
    print('SSSSS');
          ...
          children: <Widget>[
            Expanded(
              child: CupertinoPicker(
                ...
                onSelectedItemChanged: (index) {
                  setState(() {
                    selectedMarket = marketListDataNew[index];
                  });
                },
                children: new List<Widget>.generate(
                    marketListDataNew.length, (index) {
                  return new Center(
                    child: Text(
                      "${marketListDataNew[index].name}",
            ...
            Card(
              ...
              child: Padding(
                ...
                child: InkWell(
                  child: Text(
                    "Auswählen und schließen",
                    style:
                    TextStyle(fontFamily: "Bold", color: Colors.white),
                  ),
                  onTap: () {
                    Provider.of<ProductAllProvider>(context, listen: false).getProductAllList(pageNumber: 1,categoryId: selectedCategoryEntity.id);
...
class ProductWidgetView0 extends StatefulWidget {
  ...
  int index;
  MarketsAllEntity marketDataEntity;
...
class _ProductWidgetView0State extends State<ProductWidgetView0> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () => Navigator.push(context, CupertinoPageRoute(builder: (context) => ProductDetailPage(widget.productDataEntity, marketId: widget.marketDataEntity.id,),)),
      child: Container(
        padding: EdgeInsets.only(

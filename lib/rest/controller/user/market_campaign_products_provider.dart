...
class MarketCampaignProductsProvider with ChangeNotifier {

  List<ProductEntity> _marketCampaignsProductList = new List();


  List<ProductEntity> get marketCampaignsProductList =>
      _marketCampaignsProductList;

  set marketCampaignsProductList(List<ProductEntity> value) {
    _marketCampaignsProductList = value;
  }
  ...
  getMarketCampaignProductListController(int marketId,{int pageNumber=0, int orderId = 0, String search =""}) async {
    if(pageNumber==0){
      marketCampaignsProductList.clear();
    }else{
      pageToNumber++;
    ...
    if(!sharedPref.containsKey("sessionToken")){
          ...
          if (response.body != null) {
            if (response.body["result"] != null) {
              ...
              if(sessionCreateEntity!=null){
                ...
                BaseResponse response = await MarketProductService.operations().marketCampaignProducts(marketId,search ,sessionCreateEntity.result.session, page: pageNumber, order: orderId);
                try{
                  if (response is OkResponse) {
                    if (response.body != null) {
                      if (response.body["data"] != null) {
                        ...
                        marketCampaignsProductList.addAll(newDataList);
                        notifyListeners();
                      }
                    }
                  }
      ...
      BaseResponse response = await MarketProductService.operations().marketCampaignProducts(marketId,search ,sessionToken, page: pageNumber, order: orderId);
      try{
        if (response is OkResponse) {
          if (response.body != null) {
            if (response.body["data"] != null) {

...
import 'package:beysion/tabletPages/user/EmptyCartTabletWidget.dart';
...
import 'MakeYourChoiceTabletPage.dart';
import 'UserLoginTabletPageBasket.dart';

class BasketTabletPage extends StatefulWidget {
  bool widgetStatus;
  BasketTabletPage(this.widgetStatus);
  @override
  _BasketTabletPageState createState() => _BasketTabletPageState();
}

class _BasketTabletPageState extends State<BasketTabletPage> {
  ...
  _scrollListener() {
    if (basketListViewController.offset >= basketListViewController.position.maxScrollExtent && !basketListViewController.position.outOfRange) {
      setState(() {//you can do anything here
      });
    }
    if (basketListViewController.offset <= basketListViewController.position.minScrollExtent && !basketListViewController.position.outOfRange) {
      setState(() {//you can do anything here
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: buildAppBarTablet(context, homePage: !widget.widgetStatus),
                          ...
                          if(settingRepo.currentUserTokenEntity.value.token!=null){
                            if(selectedBasketDataTime.length>0){
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => MakeYourChoiceTabletPage()));
                            }else{
                            ...
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => UserLoginTabletPageBasket()));
                          }
                        },
                      ),
                    )
          ...
          child: Column(
            ...
            children: [
              ...
              Column(
                children: [
                  Row(children: [
                    Image.asset("assets/icons/lieferServiceBasket.png", width: getSize(context, true, 50/415),),
                    SizedBox(width: 5,),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Text("Durchschnittliche Lieferzeit", style: GoogleFonts.overpass(fontWeight: FontWeight.bold, fontSize: 20),),
                          Text("Die von Ihnen ausgewählten Produkte werden innerhalb von durchschnittlich ${widget.basketMarketInData.averageDeliveryTime} versendet.",
                            style: GoogleFonts.overpass(
                                fontSize: 13,
                                fontWeight: FontWeight.w600),
                            overflow: TextOverflow.fade,
                  ...
                  Container(
                    width: getSize(context, true, 1),
                    height: getSize(context, true, 80 / 415),
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          ...
                                          style: GoogleFonts.overpass(fontSize: 12, fontWeight: FontWeight.w600),
                                        ),
                                        Text(
...
/*              Container(
              ...
              ListView.separated(
                  ...
                  itemBuilder: (context, index){
                    ...
                    return BasketProductItemDataWidget(basketProduct);
                  }),
              Row(
...
class BasketProductItemDataWidget extends StatefulWidget {

  BasketProduct basketProductEntity;
  BasketProductItemDataWidget(this.basketProductEntity);

  @override
  _BasketProductItemDataWidgetState createState() => _BasketProductItemDataWidgetState();
}

class _BasketProductItemDataWidgetState extends State<BasketProductItemDataWidget> {
  @override
  Widget build(BuildContext context) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.center,
      ...
      children: [
        Flexible(
          ...
          child: Checkbox(
            visualDensity: VisualDensity.standard,
            value: widget.basketProductEntity.selectedProduct,
            onChanged: (value) {
              setState(() {
                widget.basketProductEntity.selectedProduct = value;
                Map<int, BasketProduct> selectedIds = new Map();
                selectedIds[widget.basketProductEntity.aid] = widget.basketProductEntity;
        ...
        Flexible(
          flex: 1,
          child: Image.network(
            widget.basketProductEntity.image,
        ...
        Flexible(
          flex: 3,
          child: Text("${widget.basketProductEntity.name}",
            maxLines: 3,
            softWrap: true,
            overflow: TextOverflow.fade,
            style: GoogleFonts.overpass(
        ...
        SizedBox(width: getSize(context, true, 5 / 415),),
        Container(
          child: widget.basketProductEntity.discountPrice!=null ?
          Padding(
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  child: Center(
                    child: Text(
                      fiyatYuzdeHesaplama(new ProductEntity(), basketProduct: widget.basketProductEntity),
                      overflow: TextOverflow.fade,
                      maxLines: 1,
                      softWrap: false,
                      style: GoogleFonts.overpass(
                ...
                Column(
                  children: [
                    Text(
                      "${widget.basketProductEntity.price.toString().replaceAll(".", ",")} €",
                      style: GoogleFonts.overpass(
                    ...
                    Text(
                      "${widget.basketProductEntity.discountPrice.toString().replaceAll(".", ",")} €",
                      style: GoogleFonts.overpass(
                          color: Colors.black,
                          fontSize: 17,
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      InkWell(
                        child: widget.basketProductEntity.qty != 1 ? Container(
                          width: getSize(context, true, 14 / 415),
                          height: getSize(context, true,14 / 415),
                          decoration: BoxDecoration(
                              shape: BoxShape.circle, color: BeysionColors.yellow),
                          child: Center(child: Text("-", style: GoogleFonts.overpass(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 20),)),
                        ):Container(
                          width: getSize(context, true, 16 / 415),
                          height: getSize(context, true, 16 / 415),
                          child: Center(child: Icon(Icons.delete, color: BeysionColors.yellow, size: 24,)),
                        ) ,
                        onTap: () async {
                          if(widget.basketProductEntity.qty == 1){
                            BaseResponse basketService = await BasketService.operations().deleteBasketProduct(widget.basketProductEntity.aid);
                            if(basketService is OkResponse){
                            ...
                            if(widget.basketProductEntity.qty>1){
                              int quantity = widget.basketProductEntity.qty-1;
                              BaseResponse responseData = await BasketService.operations().basketUpdate(widget.basketProductEntity.aid, quantity);
                              if (responseData.body != null) {
                                if(responseData is OkResponse){
                                  print('GÜNCELLEME BAŞARILIII');
                                  Provider.of<BasketProvider>(context, listen: false).getBasketDetailDataController();
                                  setState(()  {
                                    widget.basketProductEntity.qty--;
                                  });
                                }
                              }
                            }
                      ...
                      Container(
                        ...
                        child: Text(widget.basketProductEntity.qty.toString(), style: TextStyle(fontSize: 18),),
                      ),
                      InkWell(
                        child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Container(
                      alignment: Alignment.center,
                      width: getSize(context, true, 23 / 415),
                      height: getSize(context, true, 23 / 415),
                      child: Text(widget.basketProductEntity.qty.toString(), style: TextStyle(fontSize: 18),),
                    ),
                    InkWell(
                      child: Container(
